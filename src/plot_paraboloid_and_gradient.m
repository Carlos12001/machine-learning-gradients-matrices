function plot_paraboloid_and_gradient(A, b, range_x1, range_x2,
                stitle="Superficie Paraboloide y su Gradiente")
  %PLOT_PARABOLOID_AND_GRADIENT Grafica un paraboloide y su gradiente basado en los parámetros dados.
  %
  %   plot_paraboloid_and_gradient(A, b, range_x1, range_x2) toma como entrada una matriz A,
  %   un vector b, y rangos range_x1 y range_x2 para generar y graficar un
  %   paraboloide y su gradiente. Esta función es útil para visualizar superficies cuadráticas y su gradiente.
  %
  ...
  %   range_x1 - Vector de rango para la variable x1, utilizado para generar la malla de puntos.
  %   range_x2 - Vector de rango para la variable x2, utilizado para generar la malla de puntos.
  %
  %   Ejemplo de uso:
  %   A = [2, 1; 1, 2];
  %   b = [1; 2];
  ...
  %   plot_paraboloid_with_gradient(A, b, range_x1, range_x2);
  %
  %   Nota: Esta función asume que las dimensiones de A y b son coherentes con
  %   los requerimientos para formar una superficie cuadrática y visualizar su gradiente.
  ...
  % Calcula y grafica el gradiente
  gradient_f = X * A + repmat(b', size(X, 1), 1);
  u = reshape(gradient_f(:, 1), size(x1)); % Toma todos los valores de df/dx1
  v = reshape(gradient_f(:, 2), size(x2));% Toma todos los valores de df/dx2
  quiver(x1, x2, u, v, 'k');
  hold off;
end